...
def main():
    ...
    try:
        logger.info(f"문제 풀이 요청: '{solving_query}'")
        solution = problem_solver.solve_problem(solving_query)
        print("\n----- 문제 풀이 결과 -----")
        print(solution)
        print("-------------------------\n")
    ...
    try:
        logger.info(f"외부 검색 요청: '{search_query}'")
        search_result = external_searcher.search_and_summarize(search_query)
        print("\n----- 검색 결과 요약 -----")
        print(search_result)
        print("-------------------------\n")
    except Exception as e:
